...
with st.form("project_form"):
    ...
    target_region = st.text_input("目标地区*", placeholder="请输入目标地区，例如：济南")
    industry = st.text_input("所属产业领域", placeholder="请输入所属产业，例如：新能源")
...
if submit_button:
    ...
    if additional_file is not None:
        try:
            ...
            if "officedocument.wordprocessingml" in file_type or file_type == "application/vnd.openxmlformats-officedocument.wordprocessingml.document":
                from docx import Document
                doc = Document(BytesIO(content))
                extra_text = "\n".join(p.text.strip() for p in doc.paragraphs if p.text.strip())[:1500]
            else:
                extra_text = content.decode("utf-8", errors="ignore")[:1500]
        except Exception as e:
            st.warning(f"文件解析失败：{str(e)}，仅使用表单文字内容")

    # 正确格式化投资额
    investment_str = f"{total_investment:,.0f}" if total_investment is not None else "未填"
...
"""
    ...
    with st.spinner(f"正在生成完整政府BP及落地方案（预计 {MAX_GENERATION_SECONDS} 秒内完成）..."):
        try:
            ...
            # 简单处理：让 Streamlit 识别标题（加 markdown 兼容）
            # 如果你坚持纯文本显示，可直接 st.text_area，但体验差
