...
# ====================== 新增：司法环境与类案法律风险分析专用 System Prompt ======================
...
【类案分析方法论参考模板 —— 来自谭总提供的司法案例研究，仅作分析框架参考，不代表目标地区实际情况】
...
二、参考案例类型库（以房地产行业为例的典型纠纷场景，实际生成时需按企业所属产业类比替换
为该产业对应的高频纠纷类型，方法论保持一致）

1. 合同履约类纠纷（地产行业对应：建设工程施工合同纠纷/工程款优先受偿权）
   典型倾向：本地法院可能以"保交楼""维稳"等理由，压缩外地一方的合同法定权利范围、
   起算时间、行使期限，优先保障本地主体利益。
   参考案例：中成煤炭建设（集团）公司诉成都泓昌嘉泰房地产公司案（最高法公报案例）——
   一审弱化外地施工方优先受偿权，最高法再审纠正。
   （2021）最高法民再327号、（2020）最高法民申5074号、（2019）最高法民申6085号
   均为同类纠偏案例。

2. 合作/合资类纠纷（地产行业对应：合作开发房地产合同纠纷）
   典型倾向：本地法院可能倾向采信本地主体单方证据，在合同解除、违约金、收益分成等
   问题上作出不利于外地投资方的认定。
   参考案例：（2022）最高法民再111号、（2021）最高法民申7132号、
   （2020）最高法民终1227号。

3. 执行/权益保全类纠纷（地产行业对应：执行异议之诉，外地抵押权人/债权人 vs 本地
   购房者或本地主体）
   典型倾向：地方法院可能扩大适用消费者保护类规则，突破外地一方的优先受偿/抵押权利，
   批量支持本地主体异议。
   参考案例：（2022）最高法民申3595号、（2021）最高法民再187号 —— 最高法明确划定
   权利保护边界，纠正地方裁判扩大化倾向。

4. 破产重整/清算类纠纷
   典型倾向：本地主导的府院联动重整方案，可能优先安排本地相关方权益，压低外地债权人
   清偿比例。
   法律边界提示：破产重整清偿顺序由《企业破产法》明文规定，只要方案依法表决、法院依法
   裁定，属于合法政策权衡；只有存在管理人暗箱操作、违规调整法定清偿顺位等情形才构成违法，
   分析时需注意区分"合法政策倾斜"与"违法地方保护"两种情形，不可一概而论。
   参考案例：（2023）最高法民申2418号。

三、本模块生成时必须遵循的结构要求
1. 【类案检索方向】：根据申报企业所属产业，类比匹配该产业在跨地域经营/落地中最容易发生的
   纠纷类型（不限于地产行业，需结合实际产业特点合理类推，如制造业对应供应链/加工合同纠纷、
   科技企业对应知识产权及股权合作纠纷、涉政府项目对应政府采购/PPP合同纠纷等）。
2. 【地方保护风险初步研判】：基于公开可查的类案信息和一般司法实践规律，客观分析目标地区
   在该类纠纷中是否存在需要关注的地方保护倾向迹象（无法获得目标地区具体司法数据时，应
   明确标注为"[需实地核实/建议委托当地律师尽调]"，不得凭空捏造具体案号或数据）。
3. 【上级纠错机制评估】：说明若发生纠纷，是否存在可预期的省高院/最高法层面的救济与统一
   裁判尺度机制，作为判断该地区司法环境"可救济性"的重要依据。
4. 【结论与防控建议】：给出目标地区法治化营商环境的风险分级建议（如：低风险/中等风险/
   需重点关注），并提出可落地的防控建议（如：合同争议解决条款设计——约定仲裁或异地管辖、
   选择信誉良好的本地合作方、引入第三方履约担保机制、聘请本地及外部双重法律顾问等）。

四、强制免责与合规要求
- 本模块所有具体案号、案例仅为方法论参考模板，严禁编造目标地区不存在的具体案号或虚构
  司法数据冒充真实检索结果。
...
if submit:
    ...
    for idx, section in enumerate(SECTIONS):
        ...
        try:
            # ====== 修改点：优先使用章节自带 system prompt（如"十、司法环境"章节），
            # 未指定时沿用原有 SYSTEM_BASE，其余章节行为完全不变 ======
    ...
    for section in SECTIONS:
        ...
        if section["id"] == "zero":
            ...
            st.markdown(f"## {title}")
            st.markdown(content)
            # ====== 新增：为第十章追加一句来源说明，便于阅读方了解该模块的方法论出处 ======
            if section["id"] == "ten_legal":
                st.caption("注：本章方法论参考自类案检索框架，具体案例信息以属地律师尽调结论为准。")
        st.markdown("---")

    # 下载部分保持不变（因 SECTIONS 已新增章节，导出内容自动包含第十章）
    full_md_content = ""
    for sec in SECTIONS:
